...
func (server *UserManagementServer) CreateNewUser(ctx context.Context, in *pb.NewUser) (*pb.User, error) {
	...
	created_user := &pb.User{UserId: in.GetUserId(), Email: in.GetEmail()}
	tx, err := server.conn.Begin(context.Background())
	if err != nil {
		log.Fatalf("conn.Begin failed: %v", err)
	}

	_, err = tx.Exec(context.Background(), "insert into users(user_id, email) values ($1,$2)",
		created_user.UserId, created_user.Email)
	if err != nil {
...
func (server *UserManagementServer) GetUsers(ctx context.Context, in *pb.GetUsersParams) (*pb.UsersList, error) {
	...
	for rows.Next() {
		user := pb.User{}
		err = rows.Scan(&user.IsActive, &user.UserId, &user.Email)
		if err != nil {
			return nil, err
		}
